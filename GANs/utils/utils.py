...
def compute_gradient_penalty(D, real_images, fake_images):
    ...
    # (*, 1, 64, 64)
    # interpolated = Variable(alpha * real_images.data + ((1 - alpha) * fake_images.data)).requires_grad_(True)
    # (*,)
    ...
    # grad_l2norm = grad.norm(torch.sum(grad ** 2, dim=1))
    ...
    gradient_penalty = ((grad.norm(2,dim=1) - 1) ** 2).mean() * 5
